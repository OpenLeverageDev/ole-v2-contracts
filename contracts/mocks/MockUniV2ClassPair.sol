...
contract MockUniV2ClassPair is MockERC20 {
    ...
    constructor(address tokenA,
        address tokenB,
        uint256 reserve0,
        uint256 reserve1) MockERC20("LP", "LP")
    {
        ...
        MockERC20(_token0).mint(address(this), reserve0);
        MockERC20(_token1).mint(address(this), reserve1);
        mint(address(0));
    }
    ...
    function addReserves(address token, uint256 reserve) external{
        ...
        if(token == _token0) {
            MockERC20(_token0).mint(address(this), reserve);
            _reserve0 += reserve;
        } else {
            MockERC20(_token1).mint(address(this), reserve);
            _reserve1 += reserve;
        }
    }
